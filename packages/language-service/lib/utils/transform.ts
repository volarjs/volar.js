...
export function transformWorkspaceEdit(
	...
	{ documents, language }: ServiceContext,
	...
	for (const tsUri in edit.changeAnnotations) {
		...
		const [virtualFile] = language.files.getVirtualFile(tsUri);
	...
	if (edit.documentChanges) {
		for (const tsDocEdit of edit.documentChanges) {
			...
			if ('textDocument' in tsDocEdit) {
				...
				const [virtualFile] = language.files.getVirtualFile(tsDocEdit.textDocument.uri);
				...
				const [virtualFile] = language.files.getVirtualFile(tsDocEdit.oldUri);
				...
				const [virtualFile] = language.files.getVirtualFile(tsDocEdit.uri);
